...
`RBaM` doit être installé une fois lors de la première utilisation, puis chargé avant chaque utilisation. L'executable `BaM` doit également être téléchargé lors de la première utilisation du package.

```{r message=FALSE}
# install.packages("RBaM") Première utilisation seulement: installer le package RBaM
library(RBaM) # Charger le package 
# downloadBaM(destination_folder_on_your_computer) # Première utilisation seulement: télécharger l'executable BaM
```
