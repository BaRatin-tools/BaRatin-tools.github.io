...
`RBaM` doit être installé une fois lors de la première utilisation, puis chargé avant chaque utilisation. `BaM` doit être télécharger lors de la première utilisation du package.

```{r message=FALSE}
# install.packages("RBaM") Première utilisation: installer le package depuis CRAN
library(RBaM) # Charger le package 
# downloadBaM(dossierDestination) # Télécharger BaM lors de la première utilisation de RBaM, tout en prenant soin de préciser un dossier de destination
```
...
La première chose à faire est de définir où Bam est définir un dossier faisant office d'espace de travail (workspace), dans lequel seront sauvegardés tous les resultats.

```{r, message=FALSE}
# setPathToBaM("~/dev/BaM/makefile") #à executer à la première utilis
...
BaM(workspace=workspace,mod=M,data=D) # Estimer les paramètres du modèle M en utilisant les données de calage D
```
...
BaM(workspace=workspace, # espace de travail
    mod=M,data=D, # modèle et données
...
maxpost=prediction(X=ht['stage'],spagFiles='Qt_maxpost.spag',data.dir=workspace,
                  doParametric=FALSE,doStructural=FALSE)
# lancer BaM en mode prédiction
BaM(workspace=workspace,mod=M,data=D,pred=list(totalU,paramU,maxpost),doCalib=FALSE,doPred=TRUE)
```
...
hU=prediction(X=list(htrep),spagFiles='Qt_hU.spag',data.dir=workspace,
                  doParametric=FALSE,doStructural=FALSE)
# lancer BaM en mode prédiction
BaM(workspace=workspace,mod=M,data=D,pred=list(totalU,param_hU,hU),doCalib=FALSE,doPred=TRUE)
```
