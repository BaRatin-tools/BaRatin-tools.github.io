...
`RBaM` needs to be installed once then loaded every time it is used. `BaM` needs to be downloaded at the first usage of the package.

```{r message=FALSE}
# install.packages("RBaM") First usage: install the package from CRAN
library(RBaM) # Load package
# downloadBaM(distFolder) # Download BaM during the first usage of RBaM, be sure to point distFolder to where you would like to install it
```
...
The first thing to do is to define its workspace, i.e. the folder where all result files will be written.
...
BaM(workspace=workspace,mod=M,data=D) # Estimate the parameters of model M using calibration data D
```
...
BaM(workspace=workspace, # the bam workspace
