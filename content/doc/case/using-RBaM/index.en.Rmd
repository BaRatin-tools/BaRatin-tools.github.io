...
`RBaM` needs to be installed once then loaded every time it is used. `BaM` executable also needs to be downloaded at the first usage of the package.

```{r message=FALSE}
# install.packages("RBaM") First use only: install the package RBaM from CRAN
library(RBaM) # Load package
# downloadBaM(destination_folder_on_your_computer) # First use only: download BaM executable
```
