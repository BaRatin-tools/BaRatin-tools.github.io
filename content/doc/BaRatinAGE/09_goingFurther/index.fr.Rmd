...
# Utilisation du contenu du fichier .bam

Le fichier de sauvegarde au format .bam n'est en fait rien d'autre qu'un fichier zip qui contient l'intégralité des informations générées par BaRatinAGE, sous la forme de simples fichiers textes. Par exemple, utilisez votre application ZIP favorite pour dézipper le fichier nommé Aisne_Verrieres.bam dans le répertoire exemple. Vous obtenez alors une arborescence de dossiers qui suit exactement l'organisation des objets dans BaRatinAGE. En parcourant ces dossiers, vous pouvez alors récupérer les fichers textes bruts qui vous intéressent, et les traiter comme bon vous semble.

Malheureusement, la documentation de ces fichiers bruts est pour le moment rudimentaire. Nous améliorerons cette documentation dans les futures versions de BaratinAGE (via des en-têtes plus explicites dans les fichiers, et la rédaction d'une aide spécifique). En attendant, vous pouvez toujours envoyer un courriel à l'équipe de développement de BaRatinAGE si vous souhaitez en savoir plus sur ces fichiers.


# Pilotage de l'exécutable BaM.exe

L'environnement de travail BaRatinAGE 3 travaille en pilotant le fichier exécutable BaM.exe, qui effectue tous les calculs, et qui se trouve dans le répertoire "exe" du dossier d'installation de BaRatinAGE. Le pilotage de cet exécutable s'effectue intégralement en manipulant quelques fichiers de configuration qui sont en pratique de simples fichiers textes. Ceci implique que vous pouvez si vous le souhaitez piloter directement l'exécutable BaM.exe, sans passer par l'interface BaRatinAGE. Ceci peut être utile par exemple si vous utilisez un langage de programmation et souhaitez effectuer des calculs directement dans un de vos programmes.

Vous pouvez toujours envoyer un mail à l'équipe de développement de BaRatinAGE si vous souhaitez en savoir plus.
...
[Masafu, C., Williams, R., Satellite video remote sensing for flood model validation. Water Resources
Research, 60, e2023WR034545, 2024.](https://doi.org/10.1029/2023WR034545)
