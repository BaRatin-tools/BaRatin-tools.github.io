...
Le menu *Fichier* propose deux fonctionnalités utiles :

* *Importer un projet BaRatinAGE v2.x* : il est possible d'importer un fichier étude *.bar.zip* enregistré avec le logiciel BaRatinAGE v2, prédécesseur de l'actuel logiciel BaRatinAGE v3.

* *Exporter un rapport* : création et export d’un rapport d'étude (format PDF, DOCX, Markdown ou HTML), reprenant les principales informations et graphiques du projet, selon la sélection faite par l'utilisateur.

![](Export_Report.fr.png)
...
-   *Changer de langue* : langue de BaRatinAGE (plus de 25 langues disponibles à ce jour, n'hésitez pas à en ajouter ! Voir les explications dans la page [*Pour aller plus loin*](/fr/doc/baratinage/aller-plus-loin)).

-   *Préférences* : Les préférences globales s’appliquent par défaut à tous les projets, sauf si un projet a ses propres préférences. Les préférences sont en italique si elles n'ont pas été modifiées. Les préférences portent sur l'apparence et le comportement de BaRatinAGE : langue, mode sombre, taille de police, taille des icônes, cacher les sorties de la console de BaM, fermer cette fenêtre si BaM termine sans erreur, choix entre les coefficients de Strickler ou de Manning pour les coefficients de frottement, nombre d'échantillons utilisés pour la propagation, etc.

<!--- Options à venir, développements en cours :

-   *Préférences* : choix entre plusieurs coutumes régionales (choix du système d'unités, etc.) ;
