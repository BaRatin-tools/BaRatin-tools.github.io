...
The *File* menu offers two useful features:

* *Import a BaRatinAGE v2.x project*: you can import a *.bar.zip* study file saved with BaRatinAGE v2 software, the predecessor to the current BaRatinAGE v3 software.

* *Export a report*: create and export a study report (PDF, DOCX, Markdown or HTML format) containing the main information and graphs from the project, according to the user's selection.

![](Export_Report.en.png)
...
- *Change language*: select BaRatinAGE language (more than 25 languages available to date, feel free to add another language! See explanations in the [*Going further*](/en/doc/baratinage/going-further) page).

- *Preferences*:Global preferences apply to all projects unless a project has its own specific preferences. If shown in italics, the preferences have not been edited. Preferences relate to the appearance and behaviour of BaRatinAGE: language, dark mode, font size, icon size, hide BaM console outputs, close this window if BaM finishes without errors, choice between Strickler or Manning coefficients for friction coefficients, number of samples used for propagation, etc.
