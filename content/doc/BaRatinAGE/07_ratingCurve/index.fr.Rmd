...
-   en effectuant un clic droit sur le noeud ![](rating_curve.svg) *Courbe de tarage* dans l'arborescence de l'Explorateur des composants ;
-   en cliquant sur le bouton ![](rating_curve_add.svg) dans la barre d'outils.
...
Le graphique des traces MCMC représente les valeurs de paramètres simulées depuis la distribution a posteriori grâce au simulateur MCMC (tous les détails techniques sont ici XXX Lien ? XXX). Notez que le graphique ne montre que 500 valeurs simulées, mais en fait 10 000 valeurs ont été initialement générées, les 5 000 premières étant éliminées. Ces 5 000 valeurs initiales ont simplement été décimées et réduites à 500 valeurs pour éviter de conserver un nombre de simulations trop important. 
