...
-    Un jeu de jaugeages (utilisés pour estimer la courbe de tarage), ou éventuellement aucun (les résultats seront alors très proches des a priori) ;
...
A partir de la version 3.3 de BaRatinAGE, un onglet *Résidus* permet d'afficher les écarts des jaugeages à la courbe de tarage MaxPost (en % de débit) en fonction de la hauteur d'eau, du débit observé (jaugeages), du débit simulé (courbe de tarage) ou de l'indice du jaugeage.

![](RC_Verrieres_Residuals.fr.png)
...
## Comparateur de courbes de tarage

A partir de la version 3.1, vous pouvez créer un nouveau composant *Comparateur de courbes de tarage* permettant de tracer deux courbes de tarage dans un même graphique configurable, et ce de plusieurs façons :

-   via le menu *Composants...Créer un comparateur de courbes de tarage* ;
-   en effectuant un clic droit sur le noeud <img src="tools.svg" style="display:inline-block"> *Outils* dans l'arborescence de l'Explorateur des composants, puis sélectionner *Comparateur de courbes de tarage* ;
-   en cliquant sur le bouton <img src="comparing_rating_curves_add.svg" style="display:inline-block"> dans la barre d'outils.

![](RC_Verrieres_Comparison.fr.png)
Il vous sera possible de renommer ce comparateur de courbes de tarage et d'en saisir une description. Un comparateur de courbes de tarage existant peut être dupliqué ou supprimé.

Vous pouvez configurer le graphique en sélectionnant les deux courbes de tarage (a priori ou a posteriori) que vous souhaitez superposer, modifier les étiquettes des axes et d'autres propriétés du graphique. En sélectionnant chaque élément affiché (dans la liste), vous pouvez choisir de :

-   l'afficher ou pas sur le graphique ;
-   l'inclure ou pas dans la légende ;
-   modifier son intitulé dans la légende ;
-   modifier son apparence (couleur, ligne, transparence).   
