...
- A set of gaugings (used to estimate the rating curve), or possibly none (then, the results will be very similar to the priors);
...
Starting with version 3.3 of BaRatinAGE, a *Residuals* tab allows you to display the deviations of the gaugings from the MaxPost rating curve (as a percentage of discharge) with, on the horizontal axis, the stage, the observed discharge (gaugings), the estimated discharge (rating curve) or the gauging index.

![](RC_Verrieres_Residuals.en.png)
...
## Rating curves comparator

From version 3.1 onwards, you can create a new *Rating curves comparator* component to plot two rating curves in the same configurable graph, in several ways:

- via the *Components...Create a rating curves comparator* menu;
- by right-clicking on the <img src="tools.svg" style="display:inline-block"> *Tools* node in the Component Explorer tree, then selecting *Rating curves comparator*;
- by clicking on the <img src="comparing_rating_curves_add.svg" style="display:inline-block"> button in the toolbar.

![](RC_Verrieres_Comparison.en.png)
You can rename this rating curves comparator and enter a description. An existing rating curves comparator can be duplicated or deleted.

You can configure the graph by selecting the two (prior or posterior) rating curves you wish to superimpose, change the axis labels and other graph properties. By selecting each item displayed (from the list), you can choose to:

- whether or not to display it on the graph;
- whether or not to include it in the legend;
- change its title in the legend;
- change its appearance (colour, line, transparency).  
