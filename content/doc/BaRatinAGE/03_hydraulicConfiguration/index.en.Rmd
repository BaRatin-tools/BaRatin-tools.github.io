...
You can enter uncertain values for the physical parameters in the fields at the top of the window. Some fields are pre-filled because they depend only on the type of control, but you can always change them if you wish. 

Note that throughout BaRatinAGE, uncertainties are expressed as 95% expanded uncertainties, i.e. as the half-length of a 95% probability interval. For a Gaussian distribution, this corresponds to 1.96 times the standard deviation. 

Using the example of the Aisne at Verrières, the first control (weir) can be specified as follows:

![](SpecPriors_Verrieres.en.png)


<br>
Once the fields for all the physical parameters of a control have been filled in, the values and uncertainties of the parameters $\kappa$, $a$ and $c$ are propagated to the fields at the bottom of the window. 

It is also possible to directly specify the prior distributions of the $\kappa$, $a$ and $c$ parameters, by clicking on the *Switch to k, a, c parameters mode* button. In this case, unlike the *physical parameters mode*, it is possible to specify a distribution type other than Gaussian (from the choices offered in the drop-down menu), and the parameters of each distribution as well as the initial value that must be specified for the MCMC simulations. When using a Gaussian distribution, the standard deviation is equal to the expanded uncertainty used in the physical parameters divided by 1.96. By default, the initial value is calculated automatically by taking the median of the distribution. You can also select "Fixed value": in this case, the parameter will not be estimated but fixed at the specified value.
...
# 'Q=f(h)' hydraulic configuration 

Since version 3.1 of BaRatinAGE, you can write your own rating curve equation by creating a new 'Q=f(h)' hydraulic configuration in several ways:

- via the menu *Components... Create new Q=f(h) hydraulic configuration*;
- by right-clicking on the <img src='hydraulic_config.svg' style='display:inline-block'> *Hydraulic configuration* node in the explorer tree;
- by clicking on the <img src='hydraulic_config_qfh_add.svg' style='display:inline-block'> button in the toolbar.

You will be able to rename this new 'Q=f(h)' hydraulic configuration and enter a description. An existing 'Q=f(h)' hydraulic configuration can be duplicated or deleted.

In general, the tabs and operation of a 'Q=f(h)' hydraulic configuration are similar to those of a standard BaRatin hydraulic configuration. The main difference is that instead of defining the rating curve equation via the matrix of controls, the equation is entered directly, after selecting the 'Custom equation' option from the drop-down menu of the 'Rating curve equation' tab. Typically, the equation for a standard BaRatin control $Q=a(h-b)^c$ will be written as follows:

![](Qfh.en.png)


The name of the stage variable ($h$, typically) must be defined. By the way, note the usefulness of the *ispos(...)* function, to be placed in the term raised to the power $c$, to have zero discharge when $h<b$.

Being able to write one's own equation $Q=f(h)$ makes it possible to use controls whose formulation is not permitted by the general equation of the standard BaRatin model. 

A number of preset 'Q=f(h)' hydraulic configurations are available in the drop-down menu, including:

- trapezoidal (or even rectangular) channel control without the condition of a width much greater than the water depth ($B_w>5(h-b)$) imposed for BaRatin's rectangular channel control
- various weirs with non-rectangular notches, a situation which is difficult to represent effectively with the BaRatin model (see this [case study](/en/doc/case/charbonnieres)).

It is possible to add a preset to the catalogue of 'Q=f(h)' hydraulic configurations offered in the drop-down menu: the procedure is explained [here](/en/doc/baratinage/going-further). Don't hesitate to suggest case studies that might be useful to share with other users!


The *prior* parameters of a 'Q=f(h)' hydraulic configuration are specified by directly defining the parameter distributions, like in the *k-a-c parameters* mode of a standard hydraulic configuration. 
It is possible to specify a distribution type other than Gaussian (from the choices offered in the drop-down menu), and the parameters of each distribution as well as the initial value that must be specified for the MCMC simulations. When using a Gaussian distribution, the standard deviation is equal to the expanded uncertainty used in the physical parameters divided by 1.96. By default, the initial value is calculated automatically by taking the median of the distribution. You can also select "Fixed value": in this case, the parameter will not be estimated but fixed at the specified value.

The next steps, up to the calculation of the posterior rating curve and the discharge series, are the same as when using a standard hydraulic configuration.


# BAC hydraulic configuration

Since version 3.1 of BaRatinAGE, you can use the *BaRatin BAC* model (instead of the standard *BaRatin KAC* model) by creating a new BAC hydraulic configuration in several ways:

- via the menu *Components... Create a new BAC hydraulic configuration*;
- by right-clicking on the <img src='hydraulic_config.svg' style='display:inline-block'> *Hydraulic configuration* node in the explorer tree ;
- by clicking on the <img src='hydraulic_config_bac_add.svg' style='display:inline-block'> button in the toolbar.

You will be able to rename this new BAC hydraulic configuration and enter a description for it. An existing BAC hydraulic configuration can be duplicated or deleted.

Generally speaking, the tabs and operation of a BAC hydraulic configuration are similar to those of a standard BaRatin hydraulic configuration. The general equation of the rating curve is the same, and it is defined via the same matrix of controls. The difference is that instead of estimating the parameters $k$ (activation stage), $a$ (coefficient) and $c$ (exponent) for each hydraulic control, we estimate $b$ (offset), $a$ and $c$. Instead of $b$, $k$ is determined from the other parameters, based on the continuity condition of the rating curve, which requires numerical resolution that can sometimes cause problems.

For the moment, this new option is mostly a first step towards the future integration of a model enabling the estimation of a family of rating curves separated by rating shifts due to the evolution of the streambed. It is not recommended to use it unless there is a specific need for it.
