...
# Equations des contrôles-types (configuration BaRatin)

## Déversoir rectangulaire

<img src="rectangular_weir.svg" width="100%">
<p style="text-align: center;color: gray;"> Déversoir rectangulaire : vue en long (à gauche) et vue en travers de la section de contrôle (à droite). </p>
...
<p style="text-align: center;color: gray;"> Déversoir parabolique : vue en long (à gauche) et vue en travers de la section de contrôle (à droite). </p>
...
<p style="text-align: center;color: gray;"> Déversoir triangulaire : vue en long (à gauche) et vue en travers de la section de contrôle (à droite). </p>
...
<p style="text-align: center;color: gray;"> Orifice dénoyé : vue en long (à gauche) et vue en travers de la section de contrôle (à droite). </p>

L’équation de ce type de contrôle est :
$$Q(H) = C_o \sqrt{2g} A_w (H-b)^c $$

Pour chaque contrôle modélisé par un orifice dénoyé (l’écoulement remplit tout l’orifice mais sort librement à l’aval, sans influence de la hauteur d’eau aval sur la relation hauteur-débit, voir Figure ci-dessus), les paramètres non renseignés par défaut auxquels il faut affecter des a priori sont :
...
<p style="text-align: center;color: gray;"> Chenal rectangulaire large : vue en long (à gauche) et section en travers à l'échelle (à droite). </p>
...
Pour chaque contrôle modélisé par un chenal rectangulaire large, les paramètres non renseignés par défaut auxquels il faut affecter des a priori sont :
...
<p style="text-align: center;color: gray;"> Chenal parabolique large : vue en long (à gauche) et section en travers à l'échelle (à droite). </p>
...
<p style="text-align: center;color: gray;"> Chenal triangulaire : vue en long (à gauche) et section en travers à l'échelle (à droite). </p>
...
Pour chaque contrôle modélisé par un chenal triangulaire, les paramètres non renseignés par défaut auxquels il faut affecter des a priori sont :
...
# Autres types de contrôles (configuration "Q=f(h)")
...
Il existe néanmoins certains contrôles qui ne peuvent pas être modélisés de façon triviale en combinant les contrôles-type de la configuration BaRatin classique. Ce [cas d'étude](/fr/doc/case/charbonnieres)) en fournit un exemple : un seuil triangulaire dont l’évasement s'interrompt à partir d’une certaine hauteur pour être bordé de parois verticales. 

Depuis la version 3.1 de BaRatinAGE, il est possible de définir une configuration "Q=f(h)", c'est-à-dire d'écrire l'équation d'un ou plusieurs contrôles. Un ensemble de contrôles prédéfinis est proposé, dont la description est fournie ci-après.


## Déversoir à forme paramétrée

<img src="parametric_weir.svg" width="100%">
<p style="text-align: center;color: gray;"> Déversoir à forme paramétrée : vue en long (à gauche) et vue en travers de la section de contrôle (à droite). </p>

Comme pour les autres types de déversoirs, l'équation $Q=f(H)$ s’établit en appliquant le théorème de Bernoulli entre la section de mesure du niveau d'eau $H$ et la section critique. La charge dynamique liée à la vitesse d'approche étant supposée négligeable, la charge hydrostatique $H$ est proche de la charge hydraulique totale.

On considère toute forme de déversoir dont l'aire mouillée $A$ est proportionnelle au tirant d'eau $H-b$ élevé à la puissance $k$ :

$$A(H)=\frac{B_s}{kH_s^{k-1}} (H-b)^k$$
avec $B_s$ et $H_s$ respectivement la largeur et la hauteur de la section prises pour une même cote $H_0$ (quelconque). 

Ce type de forme de déversoir inclut les cas particuliers des contrôles section rectangle ($k=1$), parabole ($k=1.5$) et triangle ($k=2$) de BaRatin, ainsi que toutes les formes intermédiaires. 

L’équation de ce type de contrôle permet d'expliciter la relation directe entre le coefficient de débit $C(k)$ et l'exposant $k+1/2$ du contrôle :

$$Q(H)= C(k) \sqrt{2g} \frac{B_s}{H_s^{k-1}} (H-b)^{k+1/2}$$
avec :

$$C(k) = \frac{C_0}{\sqrt{2}} \frac{k^{k-1}}{(k+\frac{1}{2})^{k+\frac{1}{2}}}  $$
$C_0$ ($\approx 1$) est un coefficient de calage empirique, représentant l'écart entre le coefficient de débit réel et le coefficient de débit théorique, en fonction des conditions réelles d'approche, de géométrie, d'état de surface, etc.

On retrouve à peu de chose près les valeurs a priori des coefficients proposées par défaut pour les contrôles section classiques dans BaRatinAGE : 

*	déversoir rectangulaire : $C(1) = 0.385$ (valeur théorique pour seuil rectangulaire épais, arrondie à 0.4 dans BaRatinAGE)
*	déversoir parabolique : $C(1.5) = 0.217$ (arrondie à 0.22 dans BaRatinAGE)
*	déversoir triangulaire : $C(2) = 0.143$ (correspond au double dans l'équation BaRatin puisque $B_s/H_s=2 {\rm tan}(\nu/2)$, soit 0.286, qui est bien la valeur théorique seuil triangulaire épais, le 0.31 de BaRatinAGE étant la valeur pour seuil mince)

Les paramètres non renseignés par défaut auxquels il faut affecter des a priori sont :

* $b$ = l'offset (en $m$) égal à la cote du point le plus bas de la section
* $B_s$ = la largeur (en $m$) de la section à une cote $H_0$ (quelconque)
* $H_s$ = la hauteur (en $m$) de la section à la même cote $H_0$

Pour les autres paramètres, des a priori par défaut sont déjà spécifiés :

* $C_0$ = coefficient d'ajustement ($\approx 1 ± 0.05$)
* $g$ = accélération de la gravité ($\approx 9.81 ± 0.01 $ m.s$^{-2}$)
* $k$ = l'exposant de la relation entre surface mouillée $A$ et tirant d'eau $H-b$ (distribution uniforme entre 1 et 2, c'est-à-dire entre une section rectangulaire et une section triangulaire)

## Déversoir trapézoïdal

<img src="trapezoidal_weir.svg" width="100%">
<p style="text-align: center;color: gray;"> Déversoir trapézoïdal : vue en long (à gauche) et vue en travers de la section de contrôle (à droite). </p>

Le déversoir trapézoïdal est modélisé en ajoutant un déversoir triangulaire (représentant les bords du trapèze) au déversoir rectangulaire représentant le centre du trapèze. Les deux déversoirs ont la même cote $\kappa_1$.

L’équation de ce type de contrôle est donc :
...
* $\kappa_1$ = la hauteur d’activation (en $m$) égale à la cote de la pointe de l'encoche triangulaire, et à la cote de la crête du déversoir rectangulaire
* $\nu$ = l'angle d'ouverture du triangle (en degrés), qui est la somme des angles des deux triangles aux bords du déversoir trapézoïdal
* $B_r$ = la largeur déversante (en $m$) du déversoir rectangulaire, perpendiculairement à la direction de l’écoulement.

Pour les autres paramètres, des a priori par défaut sont déjà spécifiés :

* $C_t$ = coefficient de débit ($\approx 0.31 ± 0.05$) du déversoir triangulaire
* $g$ = accélération de la gravité ($\approx 9.81 ± 0.01 $ m.s$^{-2}$)
* $c_t$ = exposant pour une section critique triangulaire $\approx 2.5 ± 0.05$
* $C_r$ = coefficient de débit $\approx 0.4 ± 0.1$ du déversoir rectangulaire
* $c_r$ = exposant pour une section critique rectangulaire $\approx 1.5 ± 0.05$


## Déversoir circulaire

<img src="circular_weir.svg" width="100%">
<p style="text-align: center;color: gray;"> Déversoir circulaire : vue en long (à gauche) et vue en travers de la section de contrôle (à droite). </p>

Malgré sa géométrie simple, il n'existe pas de formulation explicite exacte de la courbe de tarage d'un déversoir circulaire. Plusieurs formulations approchées sont proposées dans la littérature. Elles fournissent des résultats similaires. Nous proposons une des plus anciennes (Addison, 1941, p. 107), parce qu'elle est classique et qu'elle prédit un débit proche de celui de l'orifice dénoyé, lorsque le déversoir circulaire est plein.
...
* $b$ = la cote (en $m$) du bas de la crête du déversoir circulaire
* $D$ = le diamètre (en $m$) du déversoir circulaire

Pour les autres paramètres, des a priori par défaut sont déjà spécifiés :

* $C_c$ = partie constante du coefficient de débit ($\approx 0.5550 ± 0.05$) du déversoir circulaire
* $c_1$ = exposant $\approx 1.975 ± 0.001$
* $a_1$ = coefficient $\approx 10.12 ± 0.1$
* $c_2$ = exposant $\approx 3.78 ± 0.001$
* $a_2$ = coefficient $\approx 2.66 ± 0.03$

## Déversoir rectangulaire avec encoche triangulaire

<img src="rectangular+triangleNotch_weir.svg" width="100%">
<p style="text-align: center;color: gray;"> Déversoir rectangulaire avec encoche triangulaire : vue en long (à gauche) et vue en travers de la section de contrôle (à droite). </p>

Le « triangle tronqué » est modélisé en ajoutant un déversoir triangulaire « négatif » au déversoir triangulaire réel pour retrancher l’excès de section mouillée quand la hauteur d’eau atteint les parois verticales. On ajoute simplement un déversoir rectangulaire classique pour modéliser les deux banquettes horizontales (de même cote $\kappa_2$) de part et d'autre de l'encoche.

L’équation de ce type de contrôle est donc :

$$ Q(H) = C_t \sqrt{2g} \tan(\nu/2) (H-\kappa_1)^{c_t}$$ 

si $\kappa_1\leq H \leq \kappa_2$
...
* $\kappa_1$ = la hauteur d’activation (en $m$) égale à la cote de la pointe de l'encoche triangulaire
* $\nu$ = l'angle d'ouverture du triangle (en degrés)
* $\kappa_2$ = la hauteur d’activation (en $m$) égale à la cote de la crête du déversoir rectangulaire
* $B_r$ = la largeur déversante (en $m$), i.e. la longueur transversale du déversoir rectangulaire, perpendiculairement à la direction de l’écoulement.

Pour les autres paramètres, des a priori par défaut sont déjà spécifiés :

* $C_t$ = coefficient de débit ($\approx 0.31 ± 0.05$) de l'encoche triangulaire
* $g$ = accélération de la gravité ($\approx 9.81 ± 0.01 $ m.s$^{-2}$)
* $c_t$ = exposant pour une section critique triangulaire $\approx 2.5 ± 0.05$
* $C_r$ = coefficient de débit $\approx 0.4 ± 0.1$ du déversoir rectangulaire
* $c_r$ = exposant pour une section critique rectangulaire $\approx 1.5 ± 0.05$
...
## Déversoir rectangulaire avec encoche trapézoïdale

<img src="rectangular+trapezeNotch_weir.svg" width="100%">
<p style="text-align: center;color: gray;"> Déversoir rectangulaire avec encoche trapézoïdale : vue en long (à gauche) et vue en travers de la section de contrôle (à droite). </p>

Le déversoir rectangulaire avec encoche trapézoïdale est modélisé en ajoutant un déversoir rectangulaire (de même cote $\kappa_1$) au déversoir rectangulaire avec encoche triangulaire, de façon à représenter l'encoche en « trapèze tronqué ». On distingue deux coefficients $C_{r1}$ et $C_{r2}$ différents pour les deux déversoirs rectangulaires car les conditions d'approche de l'écoulement (et les hauteurs de pelle) ne sont pas les mêmes en amont de l'encoche et en amont des banquettes.
...
* $\kappa_1$ = la hauteur d’activation (en $m$) égale à la cote de la pointe de l'encoche triangulaire et à la cote du fond du déversoir trapézoïdal
* $\nu$ = l'angle d'ouverture du triangle (en degrés)
* $\kappa_2$ = la hauteur d’activation (en $m$) égale à la cote de la crête du déversoir rectangulaire (banquettes)
* $B_{r1}$ = la largeur déversante (en $m$) du fond du déversoir trapézoïdal
* $B_{r2}$ = la largeur déversante (en $m$) du déversoir rectangulaire (banquettes)
...
* $g$ = accélération de la gravité ($\approx 9.81 ± 0.01 $ m.s$^{-2}$)
* $c_t$ = exposant pour une section critique triangulaire $\approx 2.5 ± 0.05$
* $C_{r1}$ = coefficient de débit $\approx 0.4 ± 0.1$ du déversoir rectangulaire corerspondant au fond du déversoir trapézoïdal
* $C_{r2}$ = coefficient de débit $\approx 0.4 ± 0.1$ du déversoir rectangulaire (banquettes)
* $c_r$ = exposant pour une section critique rectangulaire $\approx 1.5 ± 0.05$

## Déversoir triangulaire avec encoche triangulaire

<img src="triangular_triangleNotch_weir.svg" width="100%">
<p style="text-align: center;color: gray;"> Déversoir triangulaire avec encoche triangulaire : vue en long (à gauche) et vue en travers de la section de contrôle (à droite). </p>

Le déversoir triangulaire avec encoche triangulaire est modélisé comme le déversoir rectangulaire avec encoche triangulaire, en remplaçant simplement le déversoir rectangulaire par un déversoir triangulaire. On distingue deux coefficients $C_{t1}$ et $C_{t2}$ différents pour les deux déversoirs triangulaires car les conditions d'approche de l'écoulement (et les hauteurs de pelle) ne sont pas les mêmes en amont de l'encoche et en amont des banquettes.
...
* $\kappa_1$ = la hauteur d’activation (en $m$) égale à la cote de la pointe de l'encoche triangulaire
* $\nu_1$ = l'angle d'ouverture du triangle de l'encoche (en degrés)
* $\kappa_2$ = la hauteur d’activation (en $m$) égale à la cote de la pointe du déversoir triangulaire, là où les banquettes rejoignent les bords de l'encoche
* $\nu_2$ = l'angle d'ouverture du triangle des banquettes (en degrés)

Pour les autres paramètres, des a priori par défaut sont déjà spécifiés :

* $C_{t1}$ = coefficient de débit ($\approx 0.31 ± 0.05$) de l'encoche triangulaire
* $g$ = accélération de la gravité ($\approx 9.81 ± 0.01 $ m.s$^{-2}$)
* $c_t$ = exposant pour une section critique triangulaire $\approx 2.5 ± 0.05$
* $C_{t2}$ = coefficient de débit ($\approx 0.31 ± 0.05$) du déversoir triangulaire (banquettes)


## Déversoir-orifice rectangulaire

<img src="rectangular_orifice.svg" width="100%">
<p style="text-align: center;color: gray;"> Déversoir-orifice rectangulaire : vue en long (à gauche) et vue en travers de la section de contrôle (à droite). </p>

Le déversoir-orifice rectangulaire est modélisé en ajoutant un déversoir rectangulaire « négatif » au déversoir rectangulaire réel pour retrancher l’excès de section mouillée quand la hauteur d’eau atteint le plafond de l'orifice. Les deux déversoirs rectangulaires ont les mêmes largeur et coefficient, mais des cotes $\kappa_1$ et $\kappa_2$ différentes, correspondant au bas et au haut de l'orifice.
...
* $\kappa_1$ = la hauteur d’activation (en $m$) égale à la cote du bas de l'orifice rectangulaire
* $\kappa_2$ = la hauteur d’activation (en $m$) égale à la cote du haut de l'orifice rectangulaire
* $B_r$ = la largeur (en $m$) de l'orifice rectangulaire
...
<p style="text-align: center;color: gray;"> Chenal trapézoïdal : vue en long (à gauche) et section en travers à l'échelle (à droite). </p>

La formule de Manning-Strickler est appliquée à un chenal trapézoïdal quelconque (sans condition sur la largeur). La forme de la section trapézoïdale est définie par la largeur au fond $B_r$ et par le fruit $m$, c'est-à-dire la pente (par rapport à la verticale) des bords du chenal. Le fruit $m$ est la tangente de l'angle que fait la paroi avec la verticale, soit le rapport de la distance horizontale à la hauteur verticale. Pour un fruit nul ($m=0$), l'équation est celle d'un contrôle chenal rectangulaire (sans condition de largeur, contrairement au contrôle chenal rectangulaire large de BaRatin).

L’équation de ce type de contrôle est :

$$Q(H) = K_S \sqrt{S} \frac{\left[B_r (H-b) + m (H-b)^2 \right]^{c+1}}{\left[B_r + 2 (H-b) \sqrt{1+m^2} \right]^{c}} $$

Pour chaque contrôle modélisé par un chenal trapézoïdal, les paramètres non renseignés par défaut auxquels il faut affecter des a priori sont :

* $b$ = la cote moyenne du fond du chenal
* $K_S$ = le coefficient de résistance à l’écoulement (coefficient de Strickler en m$^{1/3}.$s$^{-1}$)
* $B_r$ = la largeur (en $m$) du fond du chenal
* $m$ = le fruit des bords de la section (distance horizontale sur hauteur verticale, sans dimension)
* $S$ = la pente longitudinale du chenal autour de la station (sans unité, il faut saisir
0.01 pour 1%, etc.)

* Le coefficient de Manning $n=1/K_S$ (m$^{-1/3}.$s) peut être utilisé à la place du coefficient de Strickler.

Pour le paramètre restant, des a priori par défaut sont déjà spécifiés :

* $c$ = exposant de la formule de Manning-Strickler $\approx 0.67 ± 0.05$

## Chenal circulaire

<img src="circular_channel.svg" width="100%">
<p style="text-align: center;color: gray;"> Chenal circulaire : vue en long (à gauche) et section en travers à l'échelle (à droite). </p>

La formule de Manning-Strickler est appliquée à un chenal circulaire : cas d'un écoulement uniforme à surface libre, et non en charge, dans une buse, conduite ou canalisation de section circulaire, de rayon $r$ et de cote du fond $b$. 

Malgré une géométrie très simple, l'équation de ce type de contrôle est relativement compliquée :

$$Q(H) = K_S \sqrt{S} \frac{r^{8/3}}{2^{2/3}} {\rm acos}(1-\frac{H-b}{r}) \left[ 1- \frac{{\rm sin}(2{\rm acos}(1-\frac{H-b}{r}))}{2{\rm acos}(1-\frac{H-b}{r})} \right]^{c+1} $$
si $b < H\leq b+r$.

$$Q(H) = K_S \sqrt{S} \frac{r^{8/3}}{2^{2/3}} \left[ \pi - {\rm acos}(\frac{H-b}{r}-1) \right] \left[ 1+ \frac{{\rm sin}(2{\rm acos}(\frac{H-b}{r}-1))}{2\pi - 2{\rm acos}(\frac{H-b}{r}-1)} \right]^{c+1} $$
si $b+r < H < b+2r$.

La formule ne se simplifie pas vraiment, même en considérant que ${\rm sin}(2{\rm acos}(x))=2x\sqrt{1-x^2}$. 

Pour chaque contrôle modélisé par un chenal circulaire, les paramètres non renseignés par défaut auxquels il faut affecter des a priori sont :

* $b$ = la cote du fond du chenal
* $K_S$ = le coefficient de résistance à l’écoulement (coefficient de Strickler en m$^{1/3}.$s$^{-1}$)
* $r$ = le rayon (en $m$) du chenal (ou de la conduite) circulaire
* $S$ = la pente longitudinale du chenal autour de la station (sans unité, il faut saisir
0.01 pour 1%, etc.)
...
Modéliser les contrôles reels avec des contrôles-types idéalisés nécessite d’approximer la géométrie réelle par une forme simple équivalente. Par exemple, pour estimer les paramètres a priori des contrôles rectangulaires par section ou par chenal, il faut trouver le rectangle qui fournit la meilleure approximation de la forme moyenne du radier naturel ou du chenal moyen (voir Figure ci-dessous). Attention, le chenal de contrôle "moyen" s’étend en fait à l’amont et à l’aval de la station, de sorte que l’approximation devrait être conduite à l’échelle du tronçon, à partir de sections en travers représentatives.

<img src="equivalentRectangle.fr.png" width="60%">
<p style="text-align: center;color: gray;"> Profil en travers du chenal de contrôle et principe de sa modélisation par un rectangle équivalent (en vert). </p>
